        ...
        case 104:
            ...
            case 301: //wyswietlanie apteczki
                if($wynik = zapytanieDoBazy($mojePolaczenie, $ApteczkaSelect2)){  
                    ...
                    echo "To jest Twoja apteczka nr 2";
                    echo "</div>";  
                    include "komunikat2.php"; 
            ...
            case 501: //wyswietlanie historii
                if($wynik = zapytanieDoBazy($mojePolaczenie, $HistoriaSelect2)){
                    echo "<div class=\"alert alert-primary\" role=\"alert\">";
                    echo "To jest historia Twojej apteczki 2";
                    echo "</div>";
                ...
                if($wynik = zapytanieDoBazy($mojePolaczenie, $UtylizacjaSelect2)){
                    include "inc/naglTabApteczkaUtylizacja.php";
                    while($wiersz = $wynik->fetch_assoc())
                        printf("<tr><td> %d </td><td> %s </td><td> %d </td> <td> %s </td>", $wiersz['id'], $wiersz['NazwaHandlowa'], $wiersz['Postac'], $wiersz['Waznosc']);
                        echo "</tbody></table>";    
                        //$mojePolaczenie->close();        
                    }else
                        echo $txtBladZapytania;
