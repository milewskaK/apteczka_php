    ...
    if($wierszy>0) {
        while($row = $wynik->fetch_assoc()){
            if(isset($_POST['NazwaHandlowa']) && (isset($_POST['Postac'])) && (isset($_POST['KodKreskowy']))) {
                printf("<tr><td> %d </td><td> %s </td><td> %s </td><td> %s </td><td><a href=\"dodajdoapteczki.php?id=%d\"> Dodaj do apteczki </a></td></tr>", $row["id"], $row["NazwaHandlowa"], $row["Postac"],  $row["KodKreskowy"], $row["id"]);
            }
        }
    }
}
